function renderizarFormulario(formulario){
    ...
    if(formulario == "resumoPedido"){
        ...
        else gerarMessageBox(2, "You must have at least one item in your cart to place an order!", "Ok");
    }
    else{
        $('#containerFormulario').addClass('ativo');
        $("#"+formulario).show();
...
function fazerLogin(){
    $.ajax({
        ...
        success: function (dados){
            ...
            gerarMessageBox(1, "User logged in successfully. Your authentication token expires in 60 minutes", "Proceed");
        }
    }).fail(function(err){
...
function buscarEnderecoPorCEP(){
    if($('#cep').val().length != 8) gerarMessageBox(2, "Please enter the full zip code", "Try again");
    else{
        $.ajax({
            method: "GET",
...
function cadastrarUsuario(){
    if(validarEtapa2()){
        ...
        $.ajax({
            ...
            success: function (dados){
                gerarMessageBox(1, "Registration completed successfully!!", "Proceed", true);
            }
        }).fail(function(err){
            gerarMessageBox(2, err.responseJSON.message, "Try again");
        });
...
function alterarEndereco(){
    if(validarEtapa2()){
        $.ajax({
            ...
            let endereco = dados.cep+", "+dados.cidade+" - "+dados.estado+", "+dados.bairro+", "+dados.rua+", "+dados.numero+", "+dados.complemento;
            localStorage.setItem('endereco', endereco);
            gerarMessageBox(1, "Address updated successfully!!", "Proceed");
        }).fail(function (err)  {
            tratarErro(err);
        });
    }
