...
function listarProdutos(){
    $.ajax({
       ...
       alert("Error to generate list: " +err.responseJSON.message);
    });
}
...
function criaLinha(dados){
    $('#tabelaProdutos').append(
        '<div class="linha-tabela-produtos" id="linha" name="linha">'+
            ...
            '<div class="coluna-7">'+
                '<button class="btn-editar" onclick="editar('+dados.codigo+')">Update</button>'+
                '<button class="btn-excluir" onclick="excluir('+dados.codigo+')">Delete</button>'+
            '</div>'+
        '</div>'
    );
}
...
function salvar(){
    ...
    $.ajax({
        ...
        else gerarMessageBox(1, "Product sucessfully saved!!", "Proceed");
    }).fail(function (err)  {
        tratarErro(err);
    });
}
...
function salvarImagem(codigo){
    $.ajax({
        ...
        gerarMessageBox(1, "Product sucessfully saved!!", "Proceed");
    }).fail(function (err)  {
        tratarErro(err);
    });
}
...
function excluir(codigo){
    $.ajax({
        ...
        gerarMessageBox(1, "Product deleted sucessfully!!", "Proceed");
    }).fail(function (err)  {
        tratarErro(err);
    });
}
