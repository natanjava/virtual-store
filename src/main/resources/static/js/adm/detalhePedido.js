...
function alterarStatusPedido(codigo, acao){
    ...
    $.ajax({
        ...
        gerarMessageBox(1, "Order "+verbo+" succesfully!!", "Proceed");
        listarPedidos();
    }).fail(function (err)  {
        tratarErro(err);
    });
...
function rejeitar(codigo){
    ...
    $('#formMotivoRejeicao').append(
        '<button type="button" class="confirmar-rejeicao" id="btnRejeicao" onclick="alterarStatusPedido('+codigo+', 2)">Confirm</button>'
    )
}
...
function preencherDetalhamento(dados){
    ...
    $('#footerBtnsDetalhePedido').append(
        '<button class="btn-confirmar" name="btnsDetalhe" onclick="alterarStatusPedido('+dados.codigo+', 1)">Accept</button>'+
        '<button class="btn-recusar" name="btnsDetalhe" onclick="rejeitar('+dados.codigo+')">Reject</button>'
    );

    if(dados.status != "Waiting for confirmation")  $("[name='btnsDetalhe']").css('display', "none");
